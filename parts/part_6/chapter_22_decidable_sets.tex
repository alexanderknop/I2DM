  ...
  \begin{algorithm}
      \begin{algorithmic}[1]
          \Function{$\Algorithm{A}$}{$x$}
              ...
              \State \Call{Print}{$\Algorithm{F}(y)$}
          \EndFunction
      \end{algorithmic}
...
\begin{proof}
  ...
  \Cref{theorem:enumeratable-to-decidable}.
  Let $\Algorithm{A}'$ be the algorithm semideciding $S$ and
  let $\Algorithm{A}$ be \Cref{algorithm:enumerating-from-semidecidable}.
  \begin{algorithm}
      \begin{algorithmic}[1]
      ...
      \caption{The algorithm enumerating the set that is semidecided by
        $\Algorithm{A}'$.}
      \label{algorithm:enumerating-from-semidecidable}
  \end{algorithm}
