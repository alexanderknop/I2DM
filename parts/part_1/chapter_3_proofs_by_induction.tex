...
\begin{principle}[The Induction Principle]
  ...
  \begin{description}
    ...
    \item [(the induction step)] $P(k) \implies P(k + 1)$ is true for all positive
      integers $k$.
  \end{description}
\end{principle}
...
\label{theorem:strong-induction}
  ...
  \begin{description}
    \item [(the base case)] $P(n_0 + 1)$ is true and
    \item [(the induction step)] If $P(n_0 + 1)$, \dots, $P(n_0 + k)$ are true, then
...
\begin{exercises}
  ...
  \exerciseitem Show that $\sum_{i = 1}^n \frac{1}{i (i + 1)} =
    \frac{n}{n + 1}$ for all integers $n \ge 1$.
  \exerciseitem Show that $\sum_{i = 1}^n \frac{1}{i^2} \le
  ...
  \exerciseitem Prove that $\sum_{i = 2}^n (i + 1) 2^i = n 2^{n + 1}$
  ...
  \exerciseitem Let us define $n!$ as follows: $1! = 1$ and
    ...
    % front of it
    \nomenclature[C]{$n"!$}{denotes $n \cdot (n - 1) \cdot (n - 2) \cdot \ldots
    \cdot 1$}
  \exerciseitem Show that
    $\int\limits_0^{+\infty} x^n e^{- x} ~ \mathrm{d}x = n!$
    for all $n \ge 0$.
  \exerciseitem Prove that $\sum_{i = 1}^n (i + 1) 2^i = n 2^{n + 1}$
    for all integers $n \ge 1$.
  \exerciseitem Show that $\sum_{k = 1}^n k \cdot k! = (n + 1)! - 1$.
\end{exercises}
