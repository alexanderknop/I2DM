...
\begin{enumerate}
  \item $\R$ a set of reals,
    \nomenclature[N]{$\R$}{denotes the set of all real numbers}
  \item $\Z$ the set of integers\footnote{``Z'' stands for the German word
    Zahlen (``numbers'').},
    \nomenclature[N]{$\Z$}{denotes the set of all integers}
  \item $\N$ the set of natural numbers\footnote{Note that in the literature
    ...
    \nomenclature[N]{$\N$}{denotes the set of all integers greater
    than $0$}
  \item $\Q$ a set of rational numbers,
    \nomenclature[N]{$\Q$}{denotes the set of all rational numbers}
  \item $\mathbb{C}$ a set of complex numbers.
    \nomenclature[N]{$\C$}{denotes the set of all complex numbers}
\end{enumerate}
...
\nomenclature[N]{$\emptyset$}{denotes the set that does
  not have elements}
...
\nomenclature[N]{$[n]$}{denotes the set of all the integers from $1$ to n}
...
\begin{exercises}
  ...
  \exerciseitem Let $A$ be a subset of a set $U$ we call this set a universe.
    ...
    \end{itemize}
  \exerciseitem Let us define an intersection of more than two sets as follows.
    ...
    \exerciseitem Let us define a union of more than two sets as follows.
      Let $A_1$, \dots, $A_n$ be some sets. Then
      \begin{itemize}
  ...
  \exerciseitem Let $\Omega$ be some set and $A_1, \dots, A_n \subseteq \Omega$.
    Show that $\bigcup_{i = 1}^n A_i =
      \set[{\exists i \in [n]\ x \in A_i}]{x \in \Omega}$.
  \exerciseitem Let $A_1$, \dots, $A_n$ be some sets.
    Show that $\bigcup\limits_{i = 1}^n (A_i \cap B) =
    \left(\bigcup_{i = 1}^n A_i\right) \cap B$.
  \exerciseitem Show that $A \Delta (B \Delta C) = (A \Delta B) \Delta C$.
\end{exercises}
