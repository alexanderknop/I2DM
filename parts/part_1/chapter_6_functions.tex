...
To illustrate this section let us consider the following game:
Alice have chosen a number from $1$ to $1000$. Bob want to guess the number
so he is asking ``yes'' or ``no'' questions to Alice which she answers.
...
Let us give an informal definition of the representation of an algorithm for
Bob.
\begin{description}
  \item[(the base case)]
    Let $x \in [1000]$, then $x$ represents the algorithm where Bob
    says that Alice's number is $x$.
  \item[(the recursion step case)]
    Let $A$ and $B$ be representations of the algorithms for Bob, and
    $f : [1000] \to \set{\text{``yes''}, \text{``no''}}$. Then
    $(f, A, B)$ is the representation of an algorithm for Bob.
\end{description}
Using this definition we may notice that $(f_1, (f_2, 1, 2), (f_3, 3, 4))$
is a representation of an algorithm for Bob, where
\begin{gather*}
  f_1(x) =
    \begin{cases}
      \text{``yes''} & \text{if } x \le 2 \\
...
\begin{itemize}
  \item It is clear that $1$, $2$, $3$, and $4$ are representations of an
    algorithm for Bob by the base case.
  \item Hence, by the recurstion step case, $(f_2, 1, 2)$ and $(f_3, 3, 4)$
    are representations of an algorithm for Bob by the base case.
  \item Finally, by the recurstion step case, $(f_1, (f_2, 1, 2), (f_3, 3, 4))$
    is a representation of an algorithm for Bob.
\end{itemize}
