...
$T_1$, \dots, $T_m$ such that $T_\ell = T$ and for each $i \in [m]$,
...
for each $i \in [m]$, $u_i \in B$ or $u_i = f(u_{k_1}, \dots, u_{k_\ell})$
...
\label{theorem:recursion-principle}
  ...
  \begin{description}
    ...
    \item[(recursion step)] $h(f_i(u_1, \dots, u_{\ell_i})) =
      F_i(h(u_1), \dots, h(u_{\ell_i}))$ for any $i \in [n]$ and
