...
$A_1, \dots, A_\subseteq U$ are some finite sets, then
\[
  \left| \bigcap_{i = 1}^n A_i \right| =
  \sum_{X \subseteq [n]} (-1)^{|X|}
    \left| \bigcap_{i \in X} \overline{A}_i \right|,
\]
where $\overline{A}_i = U \setminus A_i$.

\begin{proof}[Proof of Theorem~\ref{theorem:theorem:hamiltonian-algorithm}]
  ...
  compute $T(n, x)$ for all $x \in [n] \setminus X$ in $n^3$ steps. As a result,
  we may find the value of $\sum_{X \subseteq [n]} (-1)^{|X|} C_X$ in $2^n n^3$
  steps.
\end{proof}
