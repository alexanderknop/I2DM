...
\begin{chapterendexercises}
  ...
  \exercise Let $p$ be a permutation of the cyclic type $(c_1, \dots, c_n)$.
    ...
    \]
    \begin{solution}
      First we prove the following statement, if $p \in S_n$ is a cycle of
  ...
  \exercise Show that any permutation can be obtained as a product of some
    ...
    \end{solution}
  \exercise Show that any permuation from $S_n$ can be expressed as a product of 
    $(1, 2)$, \dots, $(n - 1, n)$ (each permuation can be used multiple times).
  \exercise Show that for any $n \in \N$, there are two permuations $p, q \in
    S_n$ such that any permuation from $S_n$ can be expressed as their product
    (each permuation can be used multiple times).
    \begin{solution}
  ...
  \exercise[recommended] A permutation $p$ is called a nontrivial involution if
    ...
    \begin{solution}
      Let $p \in S_n$ and $p^2 \neq \identity{\range{n}}$; it is easy to see
      that $(p^{-1})^2 \neq \identity{\range{n}}$. Therefore it is possible to
      split all the permutations from the set
      $\set[p^2 \neq \identity{\range{n}}]{p \in S_n}$ into pairs $p$ and
      $p^{-1}$. Hence, the size of the set
      $\set[p^2 \neq \identity{\range{n}}]{p \in S_n}$ is even; we may also note
      that the size of $S_n$ is also even. As a result, the size of 
      $\set[p^2 = I]{p \in S_n}$ is even as well, but it implies that 
      $\set[{p^2 = I \text{ and } p \neq I}]{p \in S_n}$ is odd.
    \end{solution}
