...
\begin{wrapfigure}{l}{4cm}
  \begin{center}
    ...
    \qrcode{https://youtu.be/dAoperLCjb8}
    \vskip 0.25cm
...
\begin{exercise}
  Prove \Cref{corollary:additive-principle}.
\end{exercise}
...
\begin{exercise}
  Prove \Cref{corollary:multiplicative-principle}.
\end{exercise}
...
\end{theorem}
\begin{proof}
  By \Cref{corollary:power-set-and-set-of-binary-strings}, $|2^X| =
  \left|\set{0, 1}^{|X|}\right|$, so it is enough to prove that
  $|\set{0, 1}^{|X|}| = 2^{|X|}$. This statement is true by
  \Cref{corollary:multiplicative-principle} since $|\set{0, 1}^{|X|}| =
...
\begin{proof}
  ...
  $n = 2$ is true by \Cref{theorem:inclusion-exclusion-principle}.
  ...
  \]
  In addition, by \Cref{theorem:inclusion-exclusion-principle},
  \begin{align*}
