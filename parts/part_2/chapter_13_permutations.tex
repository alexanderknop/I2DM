...
properties of them. We denote by $S_n$ the set of all permutations of
$[n]$.\footnote{%
  Letter $S$ is used since in the group theory this set is called
  the symmetric group.
}
\nomenclature[S]{$S_n$}{denote the set of all permutations of $[n]$}
...
Every permutation $p$ can is uniquely determined by the values $p(1)$, \dots,
...
round table? In other words he is asking how many permutations of the cyclic type
...
\begin{theorem}
  ...
  permutations of $[n]$ such that $i$ and $j$ are in the same cycle.
\end{theorem}
\begin{proof}
...
Melancholy of Haruhi Suzumiya'' there are $14$ episodes. The episodes are
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item ($n! + (n - 1)! + (n - 2)! + (n - 3)$)
      ...
      increases. This means that the permutation $p_k$ completed the $1$-cycle
