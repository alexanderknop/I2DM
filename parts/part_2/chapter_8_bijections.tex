...
\begin{marginfigure}
  {\scriptsize Bijections, Surjections, and Injections:\\\noindent
...
\begin{proof}
  ...
  \end{description}
\end{proof}
\noindent We denote $g$ from this theorem as $f^{-1}$.

Using this theorem we may notice that nonetheless that $X \times (Y \times Z)$
is not the same as $(X \times Y) \times Z$, there is a natural correspondence
...
Using Theorem~\ref{theorem:inverse-of-bijections} we may derive a way to apply this
...
\begin{theorem}
  The definition of $\sum\limits_{i \in S : P(i)} f(i)$ is correct;
  i.e.
  $\sum\limits_{i = 1}^k f(g_1(i)) = \sum\limits_{i = 1}^k f(g_2(i))$
...
\begin{proof}
  ...
  \begin{equation}
    ...
    \sum\limits_{i = 1}^k f(g(i)) = \sum\limits_{i = 1}^k f(g(h(i)))
  \end{equation}
  ...
  \begin{description}
    ...
    \item[The induction step:] by the induction hypothesis,
      ...
      \[
        \sum\limits_{i = 1}^k f(g(h'(i))) = \sum\limits_{i = 1}^k f(g(h(i))).
      \]
      As a result,
      Equation~\ref{equation:summation-with-respect-to_bijection} is true.
  \end{description}
  ...
  \[
    \sum\limits_{i = 1}^k f(g_1(i)) = \sum\limits_{i = 1}^k f(g(h(i))) =
    \sum\limits_{i = 1}^k f(g_2(i)).
  \]
\end{proof}
...
\begin{exercises}
  ...
  \exerciseitem Prove Theorem~\ref{theorem:injections-surjections-inequalities}.
\end{exercises}
